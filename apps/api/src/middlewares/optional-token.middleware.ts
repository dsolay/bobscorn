...
import { InvalidTokenError, ExpiredTokenError } from '@/errors/index.js'
...
function extractToken(request: Request) {
  ...
  if (!token || token === 'Bearer undefined') return undefined

  return token.replace(/^Bearer\s+/i, '')
}

export async function OptionalTokenMiddleware(request: Request, _response: Response, next: NextFunction) {
